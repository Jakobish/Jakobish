#!/bin/bash
#############################################
# Script: forensic copy.sh
# Purpose:
#   This script performs a full forensic extraction process on all PDF files in the current directory.
#   It:
#     - Verifies the presence of required tools (mutool, qpdf, exiftool, xxd, tesseract, pdfimages, pandoc)
#     - Creates a structured output directory with timestamped folders per file
#     - Extracts:
#         - OCR outputs in multiple formats (PDF, TXT, HOCR)
#         - Cleaned and analyzed structure using mutool and qpdf
#         - Metadata using exiftool
#         - Hex dump using xxd
#         - Embedded images using pdfimages
#         - Embedded objects using mutool extract
#     - Generates a Markdown forensic report per file
#     - Converts it to a PDF using pandoc
#     - Merges the report PDF with the original PDF into a composite output
# Output:
#   - All output stored in: PDF_Organizic_Output/<filename>/<timestamp>
#   - Composite PDFs include original + appended forensic report
#############################################

# pdf_organizer_composite.sh
